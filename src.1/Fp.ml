...
sig
  ...
  (*val get_all_variables : string -> (variable list * fp list)*)
end
...
struct
  ...
  let remove_not s =
    ...
    match l with
    | [] -> invalid_arg "Empty argument error"
    | [hd] -> hd
    | _::hd'::_ -> hd'

  (*let get_all_variables s =
    let blank_reg = Str.regexp "[ \t]+" in
    let s_without_blank = Str.global_replace blank_reg "" s *)
