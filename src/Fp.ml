...
struct
  ...
  let rec fp_not f_p = 
    ...
    | F -> T
    | Not fp1 -> fp1
    | Comp (fp1,C_and,fp2) -> connect C_or (fp_not fp1) (fp_not fp2)
    | Comp (fp1,C_or,fp2) -> connect C_and (fp_not fp1) (fp_not fp2)
    | Comp (fp1,C_imp,fp2) -> connect C_and fp1 (fp_not fp2)
    | Comp (fp1,C_equi,fp2) -> connect C_or (connect C_and fp1 (fp_not fp2)) (connect C_and (fp_not fp1) fp2)
    | _ -> Not f_p
  and connect (c:connector) (a:fp) (b:fp) =
    match c with
    |C_and -> 
      begin
        match (a,b) with
    ...
    |C_equi -> 
      begin
        ...
        | (F,b) -> fp_not b
        | (a,F) -> fp_not a
        | _ -> connect C_and (connect C_imp a b) (connect C_imp b a )
      end
